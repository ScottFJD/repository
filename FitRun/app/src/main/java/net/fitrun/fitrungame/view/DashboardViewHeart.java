 ...
 */
public class DashboardViewHeart extends View {
    ...
    private int minScope = 100; //最小范围
    private int maxScope = 160; //最大范围
    ...
    private float unitAngle ; //每个刻度的扫描范围
    float oneEndAng;
    float twoEndAng;
    ...
    public DashboardViewHeart(Context context) {
        this(context, null);
    }

    public DashboardViewHeart(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public DashboardViewHeart(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private void init() {
        ...
        float sunNumber = mMax - mMin;
        unitAngle = mSweepAngle/sunNumber;
        oneEndAng = unitAngle * (minScope - mMin);
        twoEndAng = unitAngle * (maxScope - mMin);
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        mRectFInnerArc.set(
                getPaddingLeft() + mLength2 + mRectText.height() + dp2px(60),
                getPaddingTop() + mLength2 + mRectText.height() + dp2px(60),
                getMeasuredWidth() - getPaddingRight() - mLength2 - mRectText.height() - dp2px(60),
                getMeasuredWidth() - getPaddingBottom() - mLength2 - mRectText.height() - dp2px(60)
        );
    ...
    protected void onDraw(Canvas canvas) {
       ...
       /* canvas.save();
        ...
        mPaint.setColor(ContextCompat.getColor(getContext(), R.color.dash_end_color));
        canvas.drawArc(mRectFInnerArc, mStartAngle + 1, mSweepAngle - 2, false, mPaint);
        mPaint.setColor(ContextCompat.getColor(getContext(), R.color.dash_center_color));
        canvas.drawArc(mRectFInnerArc, mStartAngle + 1, twoEndAng - 2, false, mPaint);
        mPaint.setColor(ContextCompat.getColor(getContext(), R.color.dash_start_color));
        canvas.drawArc(mRectFInnerArc, mStartAngle + 1, oneEndAng - 2, false, mPaint);
...
*/
        ...
        if (mVelocity < minScope){
            mPaint.setColor(ContextCompat.getColor(getContext(), R.color.dash_start_color));
            canvas.drawLine(p1[0], p1[1], mCenterX, mCenterY, mPaint);
        }else if (minScope < mVelocity && mVelocity <= maxScope ){
            mPaint.setColor(ContextCompat.getColor(getContext(), R.color.dash_center_color));
            canvas.drawLine(p1[0], p1[1], mCenterX, mCenterY, mPaint);
        }else {
            mPaint.setColor(ContextCompat.getColor(getContext(), R.color.dash_end_color));
        ...
        mPaint.setColor(ContextCompat.getColor(getContext(), R.color.color_light));
        float[] p2 = getCoordinatePoint(mPSRadius, θ);
        canvas.drawLine(p2[0], p2[1], mCenterX, mCenterY, mPaint);
